import { beginTransaction, commitTransaction, rollbackTransaction } from "../services/query.js";
...
import {
	getGameByPlayerId,
	getCountGameByPlayerId
} from "../services/gameService.js";

import { insertRequest } from "../services/gdprService.js";
...
async function gdprRoutes(fastify) {
	...
	fastify.get('/data-export', async (request, reply) => {
		try {
			...
			const friendships = await getFriendById(userId);

			const messages = await getMessageById(userId);

			const games = await getGameByPlayerId(userId);
	...
	fastify.post('/anonymize', async (request, reply) => {
		try {
			...
			await updateUserById(anonymousUsername, anonymousEmail, randomPassword, userId);
			...
			// 	});
			await updateMessageById('[Message removed]', userId);
			// 		fastify.db.prepare(`
			// 	UPDATE messages 
	...
	fastify.get('/preferences', async (request, reply) => {
		try {
			...
			reply.code(200).send({
				preferences: privacy.privacyPreferences,
				security: {
					twoFactorEnabled: Boolean(user.is_two_factor_enabled)
				}
			});
		} catch (error) {
	...
	fastify.put('/preferences', async (request, reply) => {
		try {
			...
			const userData = await getPrivacyById(userId);
			if (!userData) {
				return reply.code(404).send({ error: 'User not found' });
			}

			const privacy = JSON.parse(userData.privacy || '{}');
			privacy.privacyPreferences = {
				...privacy.privacyPreferences,
				...preferences
			};

			await updatePrivacyById(JSON.stringify(privacy), userId);
	...
	fastify.post('/request', async (request, reply) => {
		try {
			...
			await insertRequest(userId, userEmail, requestType, details || '', 'pending');
	...
	fastify.get('/overview', async (request, reply) => {
		try {
			...
			const friendCount = await getCountFriendById(userId);
			const messageCount = await getCountMessageById(userId);
			const gameCount = await getCountGameByPlayerId(userId);
